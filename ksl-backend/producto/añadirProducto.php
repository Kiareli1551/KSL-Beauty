...
if (empty($data["nombreProducto"]) || empty($data["descripcionProducto"]) || 
    empty($data["precioProducto"]) || empty($data["cantidadProducto"]) || 
...
try {
    ...
    $queryMaxProducto = "SELECT MAX(idProducto) as maxId FROM producto";
    $resultMaxProducto = $conn->query($queryMaxProducto);
    $rowMaxProducto = $resultMaxProducto->fetch_assoc();
    $nuevoIdProducto = ($rowMaxProducto["maxId"] ?? 0) + 1;
